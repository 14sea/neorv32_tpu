#!/usr/bin/env python3
"""
boot_linux.py — Boot nommu Linux + NPU on NEORV32 AX301 (standalone, no U-Boot)
...
def main():
    ...
    dtb = args.dtb or os.path.join(base, "output", "neorv32_tpu.dtb")
    ...
    if not args.skip_stage2:
        ...
        if not args.skip_program:
            rbf = args.rbf or os.path.join(base, "output", "neorv32_tpu.rbf")
            if not os.path.exists(rbf):
                rbf = os.path.join(base, "neorv32_tpu.rbf")  # fall back to project-root location
            # Locate openFPGALoader: prefer shared build, else $PATH
            loader = os.path.expanduser("~/see_neorv32_run_linux/tools/openFPGALoader/build/openFPGALoader")
